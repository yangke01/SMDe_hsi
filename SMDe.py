...
class Discriminator(nn.Module):
    ...
    def forward(self, x):
        spe = torch.mean(x, dim=(2, 3))
        diff = torch.diff(spe, dim=1)
        x = torch.cat((spe, diff), dim=1)
...
class Spectral_Smoother(nn.Module):
    ...
    def forward(self, x):
        b, c, h, w = x.shape
        spe = torch.mean(x, dim=(2, 3)).unsqueeze(2)
        out, _ = self.gru(spe)
        out = self.fc(out).squeeze(2)
        return out


class Spectral_Strategy(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.threshold = 0.5
        self.discriminator = Discriminator(dim)
        self.smooth = Spectral_Smoother()
...
class Spectral_Attention(nn.Module):
    def __init__(self,
                 ...
                 spe_window_size, ):
        super().__init__()
        self.window_size = spe_window_size
        ...
        self.spe = Spectral_Strategy(dim=dim)
        # self.spe = Spectral_Smoother()

    def forward(self, x, hsi):
        b, c, h, w = x.shape
        x_size = (x.shape[2], x.shape[3])

        x = self.patch_embed(x)
        hsi = self.patch_embed(hsi)

        x = x.reshape(b, h, w, c)
        hsi = hsi.reshape(b, h, w, c)

        x_windows = window_partition(x, self.window_size)
        x_windows = x_windows.view(-1, self.window_size * self.window_size, c)

        hsi_windows = window_partition(hsi, self.window_size)
        hsi_windows = hsi_windows.permute(0, 3, 1, 2)
        hsi = self.spe(hsi_windows)  # nB,C

        res = x_windows  # nB,Ph*Pw,C
        x_windows = self.fc1(x_windows) * hsi.unsqueeze(1)
        x_windows = self.fc2(x_windows) + res
...
class SMDe(nn.Module):
    def __init__(self,
        ...
        self.SA = Spectral_Attention(num_out_ch, input_resolution=512, spe_window_size=16)
        self.proj_out = nn.Conv2d(2 * num_out_ch, num_out_ch, 3, 1, 1)
    ...
    def forward(self, x):
        ...
        x = self.proj_out(torch.cat((out, x), dim=1))
